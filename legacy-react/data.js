...
(function () {
  ...
  // h = hora início (minutos desde 00:00), parte da agenda do dia
  const status = ['pendente','confirmado','atendimento','concluido','faltou','cancelado'];
  const hoje = [
  ...
  const caixa = { aberto: true, abertura: '08:30', valorAbertura: 200, operador: 'Carlos Menezes' };
  const lancamentos = [
  ...
  // fluxo de caixa últimos 7 dias (receita, despesa)
  const fluxo = [
  ...
  // ---------- FASE 2 · Comissões ----------
  const periodoComissao = '01 – 15 de junho';
  const comissoes = {
  ...
  const fidelidade = {
    tipo: 'pontos',          // 'pontos' | 'cashback'
    meta: 10,                // a cada 10 cortes
    recompensa: '1 corte grátis',
    cashbackPct: 5,
    ativos: 84,              // clientes no programa
    resgatados: 23,          // recompensas resgatadas no mês
    pontosEmitidos: 1240,
  };
  const campanhas = [
  ...
  const horarios = []; // grade da agenda
  ...
  window.DATA = {
    estabelecimento: { nome: 'Navalha & Cia', plano: 'Profissional', cidade: 'São Paulo · SP' },
    usuario: { nome: 'Carlos Menezes', papel: 'Dono / Admin', cor: '#0e9f6e' },
    staff, servicos, clientes, hoje, agendamentos, historico, kpis, horarios, statusLabels, status,
    produtos, movEstoque, caixa, lancamentos, aReceber, fluxo, comissoes, periodoComissao,
    fidelidade, campanhas, modelos, relatorio,
    prod: (id) => produtos.find(p => p.id === id),
    // helpers
    srv: (id) => servicos.find(s => s.id === id),
    cli: (id) => clientes.find(c => c.id === id),
    prof: (id) => staff.find(p => p.id === id),
    money: (n) => 'R$ ' + n.toLocaleString('pt-BR', { minimumFractionDigits: 0 }),
    money2: (n) => 'R$ ' + n.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
    initials: (nome) => nome.split(' ').filter(Boolean).slice(0,2).map(w => w[0]).join('').toUpperCase(),
    toMin: (hhmm) => { const [h,m] = hhmm.split(':').map(Number); return h*60+m; },
    fromMin: (min) => `${String(Math.floor(min/60)).padStart(2,'0')}:${String(min%60).padStart(2,'0')}`,
    fmtData: (iso) => { const [y,m,d] = iso.split('-'); return `${d}/${m}`; },
    diasDesde: (iso) => Math.round((new Date('2026-06-09') - new Date(iso)) / 86400000),
  };
})();
